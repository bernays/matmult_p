...
{
    int i,j;
    char buf0[20]; /* arrays which will store the arguments that will be passed to ./multiply*/
    char buf2[20];
    char buf3[20];
    char buf1[20];
    char buf4[20];
    ...
    char *bufargs[8] = {buf0,buf1,buf2,buf3,buf4,buf5,buf6,(char*)0};
    int *matrix;
...
void readmats(int inputmatrix1[20][20], int inputmatrix2[20][20], int* rows, int *columns, int *rows2, int *columns2 ){
    ...
    *columns2 = columncounter2; // The number of columns of the 2D array
    if(*columns == *rows2){
        ...
        printf("Invalid matrix dimensions: cannot multiply: Columns: %d. Rows %d\n", *columns, *rows2);
        fflush(stdout);
    }
}
